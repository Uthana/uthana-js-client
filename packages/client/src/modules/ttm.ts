...
import type { TextToMotionResult, TtmModelType } from "../types.js";
...
export class TtmModule extends BaseModule {
  ...
  async create(
    ...
    const opts = options ?? {};
    const { mutation, variables } = this._client._prepareAndSelectTextToMotion({
    ...
    const data = (await this._client._graphql<Record<string, unknown>>(
      mutation,
      variables,
